...
//!
//! This crate ships as a loadable `TinyBus` module: `crates/tinyconnectors` is built
...
//!
//! **No behavior.** The `greet` implementation lives in `crates/tinyconnectors`,
...
//! `tinyconnectors::GreetRequest` and `tinyconnectors_bus::greeting::GreetRequest` are the
...
//!
//! So: a module author depends on `tinyconnectors` and gets behavior and vocabulary.
//! A host depends on `tinyconnectors-bus` and gets vocabulary alone.
//!
//! # Staying in step with the module
//!
//! [`names::METHODS`] lists every member. `crates/tinyconnectors` asserts its served
...
//!
//! ```
//! use tinyconnectors_bus::{names, GreetRequest, GreetResponse};
//!
//! let body = serde_json::to_value([GreetRequest::new("Ferris")])?;
//! assert_eq!(names::methods::GREET, "Greet");
//! assert_eq!(names::OBJECT_PATH, "/ai/tinyhumans/connectors/Greeting");
//!
