...
//! names and payload types it serves come from [`tinyconnectors_bus`], so a host
...
#[tinybus::interface(name = "ai.tinyhumans.connectors.Greeting")]
...
tinybus_module::module_export! {
    setup = setup,
    worker_threads = 1,
    provides = ["ai.tinyhumans.connectors.Greeting"],
    methods = ["Greet"],
    signals = [],
    requires = [],
    optional = [],
