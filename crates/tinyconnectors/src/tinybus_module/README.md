...
spelled here: they come from `tinyconnectors-bus`, so a rename is a compile error in
...
`tinyconnectors_bus::names::METHODS`; the unit tests check both relationships.
Integration tests use TinyBus's in-memory transport, and
`crates/tinyconnectors/examples/verify_module.rs` loads a compiled `cdylib` through
...
declarations together — here and in `crates/tinyconnectors-bus/src/names/`. They must not retain Rust-owned data across the
