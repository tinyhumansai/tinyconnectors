...
//!
//! - [`tinyconnectors_bus`] — the wire contract. Member names, payload types, and the
//!   contract version, with no transport and no behavior. A host that only
//!   makes calls depends on that crate alone.
//! - `tinyconnectors` — this crate. The behavior, the crate-wide error type, and the
...
//!   [`tinyconnectors_bus`] — so downstream users have a single predictable surface
//!   and `tinyconnectors::GreetRequest` is the *same type* as
//!   `tinyconnectors_bus::GreetRequest`, not a structural twin.
...
//!
//! ```
//! use tinyconnectors::{greet, Error, GreetRequest};
//!
...
// publishes. A host may depend on `tinyconnectors-bus` directly and get exactly these
